...
def _trim_edge_caption_bands(page, rect):
    """Remove short title/caption bands hugging the top/bottom crop edges."""
    out = type(rect)(rect)
    min_h = 90.0
...
def _tighten_to_non_bg_pixels(page, rect, fitz_mod):
    """Trim blank margins by detecting non-background pixels in a coarse render."""
    ...
    # Require a minimum density so one stray pixel doesn't keep a full margin.
...
def extract_pdf_images(path: Path, assets_root: Path):
    ...
    for page_idx in range(len(doc)):
        ...
        # Vector fallback for PDFs with drawn figures/charts.
